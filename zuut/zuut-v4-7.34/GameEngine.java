...
public class GameEngine
{
    ...
    private void eat(final Command pSecondMot) {
        if(pSecondMot.hasSecondWord()) {//si l'utilisateur tape un second mot apres "eat" (exemple: "eat fish")
            String vItemName = pSecondMot.getSecondWord();
            Item vItem = this.aPlayer.getCurrentRoom().getItem(vItemName);
            if(vItem.getNameItem() == "Cookie") { //si le player mange un Cookie
                this.aPlayer.donneVie(20);
            } else {
                this.aGui.println("Cet Item ne peux pas être mangé.");
            }
